class ProductsController < ApplicationController
  ...
  def destroy
    respond_to do |format|
      if (@product.destroy rescue false)
        format.html { redirect_to products_url, notice: 'Product was successfully destroyed.' }
        format.json { head :no_content }
      else
        format.html { redirect_to product_url(@product), alert: 'El producto no se puede borrar, probablemente tiene ventas asociadas. Puedes pausarlo para que no aparezca en los listados.' }
        format.json { render json: @product.errors, status: :unprocessable_entity }
      end
    end
  end
  ...
  private
    ...
    def product_params
      params.require(:product).permit(:title, :status, :special, :key_code, :brand, :category_id, :stock, :price, :currency, :width_cm, :height_cm, :depth_cm, :description, :characteristics, :data_sheet, :information, :external_link, :slug)
    end
end
